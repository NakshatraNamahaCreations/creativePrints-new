// src/pages/useDesignerEngineCore.jsx
...
/**
 * useDesignerEngineCore
 * A hook containing the entire designer engine logic (Fabric canvas + template rendering).
 *
 * Exported as default. It returns the same API shape your existing code expects:
 * { templateBase, FIELD_DEFS, data, setData, CARD, canvasRef, fabricRef, ... }
 */
export default function useDesignerEngineCore(templateId, navigate) {
  // use the template resolver (findTemplate) and buildCardPx (both used — avoid ESLint unused)
  const templateBase = useMemo(() => findTemplate(templateId), [templateId]);

  // choose dimensions (uses buildCardPx later)
  ...
  // convert baseSpec to pixel card preview (buildCardPx expects spec)
  ...
  // ---------- Background helpers ----------
  // create or return existing permanent bg rect (non-selectable, non-evented)
  const createOrGetBgRect = (c) => {
    if (!c) return null;
    const existing = c.getObjects().find((o) => o?.data?.role === "background");
    if (existing) return existing;

    try {
      const cornerRadiusPx =
        CARD.cornerRadiusPx ??
        (CARD.cornerRadiusMM ? Math.round((CARD.cornerRadiusMM / 25.4) * CARD.dpi) : 0);

      const bgRect = new fabric.Rect({
        left: 0,
        top: 0,
        width: c.getWidth() || CARD.w,
        height: c.getHeight() || CARD.h,
        rx: cornerRadiusPx,
        ry: cornerRadiusPx,
        fill: (palette && palette.bg) || "#FFFFFF",
        selectable: false,
        evented: false,
        hasControls: false,
        hoverCursor: null,
      ...
      bgRect.set("data", { layer: "card-bg", role: "background" });
      c.add(bgRect);
      c.sendToBack(bgRect);
      return bgRect;
    } catch (err) {
      console.warn("createOrGetBgRect failed", err);
      return null;
    }
  };

  // ensure bg rect fills the whole canvas (call on resize)
  const ensureBgRectMatchesCanvas = (c) => {
    if (!c) return;
    const bg = createOrGetBgRect(c);
    if (!bg) return;
    bg.set({
      left: 0,
      top: 0,
  ...
  // public setter to update canvas background color/image (safe)
  const setCanvasBackground = async (payload) => {
    ...
    // prefer updating the permanent bg rect
    let bg = c.getObjects().find((o) => o?.data?.role === "background");
    if (!bg) bg = createOrGetBgRect(c);

    try {
      if (!bg) {
        // fallback to canvas background color
        if (dataPayload.mode === "color") {
          c.setBackgroundColor(dataPayload.color || null, c.requestRenderAll.bind(c));
        } else if (dataPayload.mode === "transparent") {
          c.setBackgroundColor(null, c.requestRenderAll.bind(c));
        }
        return;
      }
      ...
      if (dataPayload.mode === "color") {
        bg.set("fill", dataPayload.color || null);
        bg.set("clipPath", null);
        bg.set("opacity", 1);
        bg.setCoords();
        c.sendToBack(bg);
      ...
      if (dataPayload.mode === "image" && dataPayload.imageUrl) {
        // load image and create a pattern fill on the bg rect (no new selectable object)
        await new Promise((resolve, reject) => {
          fabric.Image.fromURL(
            dataPayload.imageUrl,
            (img) => {
              if (!img) return reject(new Error("image load failed"));
              const w = bg.width * (bg.scaleX || 1) || c.getWidth();
              const h = bg.height * (bg.scaleY || 1) || c.getHeight();
              const scaleX = w / img.width;
              const scaleY = h / img.height;
              const finalScale =
                (dataPayload.fit === "cover" ? Math.max(scaleX, scaleY) : Math.min(scaleX, scaleY)) *
                (typeof dataPayload.scale === "number" ? dataPayload.scale : 1);

              // build a canvas source for pattern
              const patternCanvas = document.createElement("canvas");
              patternCanvas.width = Math.round(img.width * finalScale);
              patternCanvas.height = Math.round(img.height * finalScale);
              const ctx = patternCanvas.getContext("2d");
              // img.getElement() may not exist for old fabric versions; handle both
              const imgEl = img.getElement ? img.getElement() : img._element || null;
              if (imgEl) ctx.drawImage(imgEl, 0, 0, patternCanvas.width, patternCanvas.height);
              const pattern = new fabric.Pattern({ source: patternCanvas, repeat: "no-repeat" });
              bg.set("fill", pattern);
              bg.setCoords();
              c.sendToBack(bg);
              c.requestRenderAll();
      ...
      try {
        ...
        console.warn("setCanvasBackground fallback failed", e);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    // find container to size to (wrap canvas in a parent or default to body)
    const canvasEl = canvasRef.current;
    const container = canvasEl.parentElement || document.body;

    const resizeCanvasToContainer = () => {
      const width = container.clientWidth || window.innerWidth;
      // choose the height you want — using at least CARD.h keeps minimum preview height
      const height = Math.max(container.clientHeight || window.innerHeight, CARD.h);

      // make canvas DOM element fill container visually
      canvasEl.style.display = "block";
      canvasEl.style.width = `${width}px`;
      canvasEl.style.height = `${height}px`;

      // set Fabric logical size to match DOM size
      c.setWidth(width);
      c.setHeight(height);

      // keep devicePixelRatio zoom if desired
      const ratio = window.devicePixelRatio || 1;
      c.setZoom(ratio);

      // ensure bg matches new size
      ensureBgRectMatchesCanvas(c);
    ...
    // ensure bg stays at back when objects are added
    const onObjectAdded = () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [CARD.w, CARD.h, /* keep other deps out to avoid recreating canvas too often */]);
  ...
  // main render effect — preserve background object
  useEffect(() => {
    const tpl = enumeratedTemplate;
    const c = fabricRef.current;
    if (!tpl || !c) return;

    // remove only template/runtime objects, preserve background
    const toRemove = c.getObjects().filter((o) => !(o?.data?.role === "background"));
    toRemove.forEach((o) => {
      try {
        c.remove(o);
      } catch {
    ...
    // ensure background exists & sized
    ensureBgRectMatchesCanvas(c);
    const bgObj = c.getObjects().find((o) => o?.data?.role === "background");
    if (bgObj) c.sendToBack(bgObj);
    ...
    (tpl.elements || []).forEach((el) => {
      ...
      if (el.type === "rect") {
        ...
        // heuristic: if base template defines a full-size white rect, make it follow palette bg
        let fillVal = resolveColor(el.fill);
        const elFillStr = String(el.fill || "").toLowerCase();
        const isBaseWhiteFull =
          el._origin === "base" &&
          (elFillStr === "#ffffff" || elFillStr === "white") &&
          (el.w === "100%" || el.h === "100%" || (_toPx(el.w, "x") >= CARD.w - 2 && _toPx(el.h, "y") >= CARD.h - 2));

        if (isBaseWhiteFull) {
          fillVal = palette?.bg || "#FFFFFF";
        }
    ...
    const onModified = (e) => {
      ...
      const elId = obj.data.elId;
      // clamp positions to CARD dims (we still store percentages relative to CARD)
  ...
  async function renderSideAndCapture(sideKey) {
    ...
    // ensure single background rect for capture view (we use CARD dims here)
    try {
      const cornerRadiusPx =
        CARD.cornerRadiusPx ??
        (CARD.cornerRadiusMM ? Math.round((CARD.cornerRadiusMM / 25.4) * CARD.dpi) : 0);
      const bgFill = (paletteObj && paletteObj.bg) || "#FFFFFF";
      ...
      console.warn("bg rect setup failed (capture)", err);
    }
    ...
    for (const el of bakedEls) {
      ...
      if (el.type === "rect") {
        ...
        // snapshot: same heuristic for full-white base rects
        let fillVal = resolveColor(el.fill);
        const elFillStr = String(el.fill || "").toLowerCase();
        const isBaseWhiteFull =
          el._origin === "base" &&
          (elFillStr === "#ffffff" || elFillStr === "white") &&
          (el.w === "100%" || el.h === "100%" || (toPx(el.w, "x") >= CARD.w - 2 && toPx(el.h, "y") >= CARD.h - 2));

        if (isBaseWhiteFull) {
          fillVal = paletteObj?.bg || "#FFFFFF";
        }
      ...
      if (el.type === "image") {
        ...
        const img = await loadImg(
          url,
          String(url).startsWith("blob:") ? {} : { crossOrigin: "anonymous" }
        );
        const iw = img.width || 1;
        const ih = img.height || 1;
